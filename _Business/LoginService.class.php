...
class LoginService {
    ...
    static function loginOut() {
        ...
        set_cookie(self::LOGIN_KEY, 0, -1);
    }

    static function delLoginInfo($mid) {
        $mc = MC();
        $mmk = self::LOGIN_KEY . "_" . $mid;
        $mc->delete($mmk);
    }

    static function getLoginUser() {
        $str = get_cookie(self::LOGIN_KEY);
        $arr = explode("###", $str);

        $mc = MC();
        $mk = self::LOGIN_KEY . "_" . $arr[0];
        $obj = $mc->get($mk);
